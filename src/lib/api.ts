...
export const couponsApi = {
  ...
  getById: (id: string | number) => api.get(`/api/coupons/${id}`),
  create: (data: any) => api.post('/api/coupons', data),
  update: (id: string | number, data: any) => api.put(`/api/coupons/${id}`, data),
  delete: (id: string | number) => api.delete(`/api/coupons/${id}`),
  toggle: (id: string | number, status: boolean) => api.put(`/api/coupons/${id}/toggle`, { status }),
  getAvailable: (cartSubtotal?: number) => api.get('/api/coupons/available', { params: { cart_subtotal: cartSubtotal } }),
  apply: (code: string, cartSubtotal: number) => api.post('/api/coupons/apply', { code, cart_subtotal: cartSubtotal }),
};
