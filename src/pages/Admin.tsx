...
import { 
  ...
  Download, Calendar, Wand2, X, Menu, Truck, CreditCard, Sparkles, RefreshCw, Upload, Bell, Ticket
} from 'lucide-react';
...
export const AdminPanel = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F5F5F7] font-sans text-[#1D1D1F]">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 w-64 bg-[#241815] border-r border-white/5 z-50 transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] lg:relative lg:translate-x-0
...
const SidebarLink = ({ icon: Icon, label, active, onClick }: any) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center space-x-3 px-4 py-2.5 rounded-xl font-bold text-xs transition-all duration-300 ${
      active 
        ...
        <div className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm space-y-8">
          ...
          <div className="space-y-4">
            ...
            <input 
              ...
              value={isNaN(codAdvance) ? '' : codAdvance}
              onChange={(e) => setCodAdvance(parseInt(e.target.value) || 0)}
            />
        ...
        <div className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm space-y-8">
          ...
          <div className="space-y-4">
            ...
            <input 
              type="number"
              className="w-full bg-gray-50 border-none rounded-xl px-6 py-4 focus:ring-2 focus:ring-black font-black text-xl"
              value={isNaN(freeShippingThreshold) ? '' : freeShippingThreshold}
...
const AdminProducts = ({ addLog }: { addLog: (msg: string) => void }) => {
  ...
  const [isVariantStockEnabled, setIsVariantStockEnabled] = useState(false);
  const [isConfirmOpen, setIsConfirmOpen] = useState(false);
  const [productToDelete, setProductToDelete] = useState<string | null>(null);

  const getVariantCombinations = () => {
    const activeSizes = sizesEnabled ? formData.sizes : [];
    const activeColors = colorsEnabled ? formData.colors : [];
  ...
  const handleVariantStockChange = (key: string, value: number) => {
    const updated = { ...variantStock, [key]: value };
    setVariantStock(updated);

    const total = Object.values(updated).reduce((sum: number, val: number) => sum + val, 0);
    setFormData(prev => ({ ...prev, stock_quantity: total }));
  };
  ...
  const handleEdit = (product: any) => {
    ...
    setSizesEnabled(product.sizes && product.sizes.length > 0);
    setColorsEnabled(product.colors && product.colors.length > 0);
    setVariantStock(product.variant_stock || {});
    setIsVariantStockEnabled(product.variant_stock && Object.keys(product.variant_stock).length > 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Auto-parse any text remaining in the color/size input fields
      let finalColors = [...formData.colors];
      if (colorsEnabled && newColor.trim()) {
        const parsed = newColor.split(',').map(c => c.trim()).filter(c => c && !finalColors.includes(c));
        finalColors = [...finalColors, ...parsed];
      }

      let finalSizes = [...formData.sizes];
      if (sizesEnabled && newSize.trim()) {
        const parsed = newSize.split(',').map(s => s.trim()).filter(s => s && !finalSizes.includes(s));
        finalSizes = [...finalSizes, ...parsed];
      }

      const activeSizes = sizesEnabled ? finalSizes : [];
      const activeColors = colorsEnabled ? finalColors : [];
      let finalCombinations: string[] = [];
      if (activeSizes.length > 0 && activeColors.length > 0) {
        activeSizes.forEach(s => {
          activeColors.forEach(c => {
            finalCombinations.push(`${s}-${c}`);
          });
        });
      } else if (activeSizes.length > 0) {
        finalCombinations = activeSizes;
      } else if (activeColors.length > 0) {
        finalCombinations = activeColors;
      }

      const finalVariantStock: Record<string, number> = {};
      if (isVariantStockEnabled && finalCombinations.length > 0) {
        finalCombinations.forEach(comb => {
          finalVariantStock[comb] = variantStock[comb] !== undefined ? Number(variantStock[comb]) : 0;
        });
      }

      const totalStock = isVariantStockEnabled && finalCombinations.length > 0
        ? Object.values(finalVariantStock).reduce((sum, v) => sum + v, 0)
        : Number(formData.stock_quantity);

      const payload = {
        ...formData,
        sizes: sizesEnabled ? finalSizes : [],
        colors: colorsEnabled ? finalColors : [],
        variant_stock: isVariantStockEnabled ? finalVariantStock : {},
  ...
  const resetForm = () => {
    ...
    setVariantStock({});
    setIsVariantStockEnabled(false);
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button 
          ...
          className="bg-brand-pink hover:bg-brand-pink-hover text-white px-8 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 transition-all active:scale-95 shadow-lg shadow-brand-pink/10"
        >
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 sm:space-y-8 pb-24 sm:pb-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          required
                          className="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black font-bold"
                          value={isNaN(formData.original_price) ? '' : formData.original_price}
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          required
                          className="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black font-bold"
                          value={isNaN(formData.discount_price) ? '' : formData.discount_price}
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        type="number"
                        required
                        disabled={isVariantStockEnabled && getVariantCombinations().length > 0}
                        className={`w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black font-bold ${isVariantStockEnabled && getVariantCombinations().length > 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
                        value={isNaN(formData.stock_quantity) ? '' : formData.stock_quantity}
                        onChange={e => setFormData({...formData, stock_quantity: parseInt(e.target.value) || 0})}
                      />
                      {isVariantStockEnabled && getVariantCombinations().length > 0 && (
                    ...
                    {role === 'superadmin' && (
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          max="365"
                          className="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black font-bold"
                          value={isNaN(formData.new_arrival_days) ? '' : formData.new_arrival_days}
                          onChange={e => setFormData({...formData, new_arrival_days: parseInt(e.target.value) || 7})}
                        />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs font-bold text-gray-800">Enable Sizes</p>
                          <p className="text-[10px] text-gray-400">Toggle size options for this product</p>
                        </div>
                        <input 
                          type="checkbox"
                          checked={sizesEnabled}
                          onChange={e => setSizesEnabled(e.target.checked)}
                          className="w-5 h-5 accent-brand-pink rounded cursor-pointer"
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs font-bold text-gray-800">Enable Colors</p>
                          <p className="text-[10px] text-gray-400">Toggle color options for this product</p>
                        </div>
                        <input 
                          type="checkbox"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className={`space-y-2 p-3 rounded-xl border border-dashed transition-all ${sizesEnabled ? 'border-gray-200 bg-transparent' : 'border-gray-100 bg-gray-50/30 opacity-50 pointer-events-none'}`}>
                        ...
                        <div className="flex gap-2">
                          <input 
                            ...
                            placeholder={sizesEnabled ? "e.g. 7, 8, XL" : "Disabled"}
                          />
                          <button type="button" disabled={!sizesEnabled} onClick={addSize} className="bg-black text-white p-2 rounded-xl disabled:bg-gray-300"><Plus className="w-4 h-4" /></button>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-2">
                          {sizesEnabled && formData.sizes.map(size => (
                      ...
                      <div className={`space-y-2 p-3 rounded-xl border border-dashed transition-all ${colorsEnabled ? 'border-gray-200 bg-transparent' : 'border-gray-100 bg-gray-50/30 opacity-50 pointer-events-none'}`}>
                        ...
                        <div className="flex gap-2">
                          <input 
                            ...
                            placeholder={colorsEnabled ? "e.g. Black, Red" : "Disabled"}
                          />
                          <button type="button" disabled={!colorsEnabled} onClick={addColor} className="bg-black text-white p-2 rounded-xl disabled:bg-gray-300"><Plus className="w-4 h-4" /></button>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-2">
                          {colorsEnabled && formData.colors.map(color => (
                    ...
                    {/* Variant Stock Allocations */}
                    {getVariantCombinations().length > 0 && (
                      <div className="space-y-4 p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-xs font-black uppercase tracking-wider text-neutral-800">Separate Stocks per Size/Color</p>
                            <p className="text-[10px] text-gray-400">Specify stock individually for each variant combination</p>
                          </div>
                          <input 
                            type="checkbox"
                            checked={isVariantStockEnabled}
                            onChange={e => setIsVariantStockEnabled(e.target.checked)}
                            className="w-5 h-5 accent-brand-pink rounded cursor-pointer"
                          />
                        </div>

                        {isVariantStockEnabled && (
                          <div className="space-y-3 pt-2 border-t border-gray-100">
                            <p className="text-[9px] font-black uppercase tracking-widest text-brand-pink">Variant Stock Allocations</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-1">
                              {getVariantCombinations().map(comb => (
                                <div key={comb} className="flex items-center justify-between gap-3 p-2.5 bg-white rounded-xl border border-gray-100 shadow-sm">
                                  <span className="text-xs font-bold text-gray-700 uppercase tracking-wide">{comb}</span>
                                  <div className="flex items-center gap-2">
                                    <input 
                                      type="number"
                                      min="0"
                                      className="w-20 bg-gray-50 border-none rounded-lg px-2 py-1 focus:ring-2 focus:ring-black font-bold text-xs text-right"
                                      value={variantStock[comb] !== undefined ? variantStock[comb] : 0}
                                      onChange={e => handleVariantStockChange(comb, Math.max(0, parseInt(e.target.value) || 0))}
                                    />
                ...
                <div className="sticky bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-gray-50 sm:static sm:p-0 sm:bg-transparent sm:border-0 sm:backdrop-blur-none">
                  <button 
                    ...
                    className="w-full bg-brand-pink hover:bg-brand-pink-hover text-white py-5 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 shadow-xl shadow-brand-pink/10"
                  >
...
const renderPositionGrid = (currentValue: string, onChange: (val: string) => void) => {
  return (
    <div className="grid grid-cols-3 gap-1 w-32 h-32 bg-gray-50 p-1.5 rounded-xl border border-gray-100 shrink-0">
      {positionOptions.map(opt => {
        ...
        return (
          <button
            ...
            className={`rounded-lg border transition-all flex flex-col items-center justify-center p-0.5 ${
              isSelected 
                ? 'border-brand-pink bg-brand-pink text-white scale-95 shadow-md shadow-brand-pink/20' 
                : 'border-gray-100 bg-white text-gray-400 hover:border-gray-300 hover:text-black'
            }`}
          >
            <div className={`w-1 h-1 rounded-full ${isSelected ? 'bg-white' : 'bg-gray-300'} mb-0.5`} />
            <span className="text-[7px] font-black uppercase tracking-tight leading-none text-center truncate w-full">{opt.label.split(' ').pop()}</span>
          </button>
        );
      })}
    </div>
...
const AdminSliders = ({ addLog }: { addLog: (msg: string) => void }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    desc_color: '#ffffff',
    desc_position: 'middle-center',
    button_position: 'middle-center'
  });
  ...
  const handleEdit = (slider: any) => {
    setFormData({
      ...
      show_description: !!slider.show_description,
      desc_color: slider.desc_color || '#ffffff',
      desc_position: slider.desc_position || 'middle-center',
      button_position: slider.button_position || 'middle-center'
    });
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-8 space-y-6 flex-1 overflow-y-auto pb-24 sm:pb-8">
                ...
                <div className="space-y-3 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                  <div className="flex justify-between items-center">
                    <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Description Font Color</label>
                      <p className="text-[9px] text-gray-400">Choose color by sliding/clicking below</p>
                    </div>
                    <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl shadow-xs border border-gray-100">
                      <div className="w-5 h-5 rounded-full border border-gray-200" style={{ backgroundColor: formData.desc_color }} />
                      <span className="text-[10px] font-mono font-bold text-gray-600 uppercase">{formData.desc_color}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-4">
                    {/* HTML5 color picker acting as a sliding spectrum control */}
                    <input 
                      type="color" 
                      className="w-12 h-10 p-0 border-0 rounded-lg cursor-pointer shrink-0 bg-transparent"
                    ...
                    <input 
                      type="text"
                      className="flex-1 bg-white border border-gray-100 rounded-xl px-4 py-2 text-xs font-bold font-mono uppercase text-gray-700"
                      value={formData.desc_color}
                      onChange={e => setFormData({...formData, desc_color: e.target.value})}
                      placeholder="#FFFFFF"
                    />
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      ...
                      { name: 'Warm Cream', value: '#FDF6EE' },
                      { name: 'Rose Gold', value: '#E8C5C8' },
                      { name: 'Brand Pink', value: '#E39B95' },
                      { name: 'Champagne Gold', value: '#E5D3B3' },
                      { name: 'Midnight', value: '#1A110E' }
                    ].map(preset => (
                      <button
                        key={preset.value}
                        type="button"
                        onClick={() => setFormData({...formData, desc_color: preset.value})}
                        className={`px-2.5 py-1 rounded-lg text-[9px] font-bold border transition-all ${formData.desc_color.toLowerCase() === preset.value.toLowerCase() ? 'border-brand-pink bg-brand-pink-light/30 text-brand-pink font-black' : 'border-gray-100 bg-white text-gray-500 hover:border-gray-300'}`}
                      >
                        {preset.name}
                      </button>
                    ))}
                  ...
                  {formData.description && (
                    <div className="p-3 rounded-xl text-center bg-[#241815] flex items-center justify-center min-h-[50px] border border-white/5">
                      <p style={{ color: formData.desc_color }} className="text-xs font-bold tracking-wide shadow-sm">
                        {formData.description}
                      </p>
                    </div>
                  )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
                  <div className="space-y-3 flex flex-col items-center sm:items-start text-center sm:text-left">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-neutral-800">Description Position</span>
                      <p className="text-[9px] text-gray-400 mt-0.5">Toggle 3x3 layout alignments</p>
                    </div>
                    {renderPositionGrid(formData.desc_position, (val) => setFormData(prev => ({ ...prev, desc_position: val })))}
                    <span className="text-[10px] font-mono font-bold text-gray-500 uppercase">
                      Current: {formData.desc_position.replace('-', ' ')}
                    </span>
                  </div>

                  <div className="space-y-3 flex flex-col items-center sm:items-start text-center sm:text-left">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-neutral-800">Button Position</span>
                      <p className="text-[9px] text-gray-400 mt-0.5">Toggle 3x3 layout alignments</p>
                    </div>
                    {renderPositionGrid(formData.button_position, (val) => setFormData(prev => ({ ...prev, button_position: val })))}
                    <span className="text-[10px] font-mono font-bold text-gray-500 uppercase">
                      Current: {formData.button_position.replace('-', ' ')}
                    </span>
                  </div>
                </div>
