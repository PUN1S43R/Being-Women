...
export const Cart = () => {
  const { cart, removeFromCart, updateQuantity, total, syncCartPricesAndStock } = useCart();
                ...
                <div className="flex flex-col pt-4">
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center border border-gray-100 rounded-full px-2 py-1">
                      <button 
                        ...
                        className={`p-1 rounded-full ${item.quantity <= 1 ? 'text-gray-300 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`p-1 rounded-full ${item.maxStock !== undefined && item.quantity >= item.maxStock ? 'text-gray-300 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                      >
                  ...
                  {item.maxStock !== undefined && item.quantity >= item.maxStock && (
                    <p className="text-[10px] text-amber-600 font-bold uppercase tracking-wider mt-2">
                      Maximum available stock reached ({item.maxStock} pieces)
                    </p>
                  )}
                </div>
              </div>
