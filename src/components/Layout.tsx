  ...
  return (
    <div className="flex flex-col items-start select-none font-serif">
      <div className="flex items-baseline leading-none">
        ...
        <span className={`text-xl font-light tracking-widest lowercase ml-1 ${light ? 'text-brand-pink-light' : 'text-brand-brown dark:text-neutral-200'}`}>women</span>
        <span className={`text-xs ml-0.5 ${light ? 'text-brand-pink-light' : 'text-brand-pink'}`}>✿</span>
      </div>
      <span className={`text-[6.5px] tracking-[0.2em] uppercase font-sans -mt-0.5 ${light ? 'text-white/60' : 'text-neutral-500 dark:text-neutral-400'}`}>
...
export const Navbar = () => {
  ...
  const isPathActive = (path: string, tag?: string, categoryId?: string) => {
    ...
    if (categoryId) {
      return location.pathname === '/products' && currentCategory === categoryId;
    }
    if (path === '/') {
      return location.pathname === '/';
    }
    if (path === '/products') {
      return location.pathname === '/products' && !currentTag && !currentCategory;
    }
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 w-full bg-white/70 dark:bg-neutral-950/65 backdrop-blur-2xl backdrop-saturate-150 border-b border-brand-pink/15 dark:border-neutral-800/40 shadow-xs transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Link to="/account" className="hidden lg:inline-flex p-2 text-neutral-600 hover:text-brand-pink hover:bg-brand-pink-light rounded-full transition-all">
        ...
        {isMenuOpen && (
          <>
            <motion.div 
              ...
              className="fixed inset-0 bg-neutral-950/40 dark:bg-neutral-950/60 backdrop-blur-sm z-[60]"
            />
            <motion.div 
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              ...
              className="fixed inset-y-0 left-0 w-80 bg-white/75 dark:bg-neutral-900/70 backdrop-blur-2xl backdrop-saturate-150 z-[70] shadow-[0_8px_32px_0_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.45)] overflow-y-auto border-r border-brand-pink/10 dark:border-neutral-800/40 flex flex-col justify-between"
            >
              <div className="p-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <Link 
                    ...
                    className={`flex items-center space-x-3 px-3 py-3 rounded-xl transition-all group ${
                      isPathActive('/') 
                        ...
                        : 'hover:bg-brand-pink-light text-neutral-700'
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                      isPathActive('/') 
                        ? 'bg-brand-pink text-white' 
                        : 'bg-brand-pink-light text-brand-pink group-hover:bg-brand-pink group-hover:text-white'
                    }`}>
                      <Home className="w-4.5 h-4.5" />
                    </div>
                    <span className={`text-xs tracking-wider ${
                      isPathActive('/') ? 'font-extrabold text-brand-pink' : 'font-bold text-neutral-700 group-hover:text-brand-pink'
                    }`}>
                      Home
                    </span>
                  </Link>
                  ...
                  >
                    ...
                    <span className={`text-xs tracking-wider ${
                      isPathActive('/products') ? 'font-extrabold text-brand-pink' : 'font-bold text-neutral-700 group-hover:text-brand-pink'
                    }`}>
                      Shop All
                    </span>
                  </Link>
                  ...
                  <Link 
                    ...
                    onClick={() => setIsMenuOpen(false)} 
                    className={`flex items-center space-x-3 px-3 py-3 rounded-xl transition-all group ${
                      isPathActive('/products', '@599') 
                        ? 'bg-brand-pink-light/80 text-brand-pink' 
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                      isPathActive('/products', '@599') 
                        ? 'bg-brand-pink text-white' 
                        : 'bg-brand-pink/10 text-brand-pink group-hover:bg-brand-pink group-hover:text-white'
                    }`}>
                      <Star className="w-4.5 h-4.5" />
                    </div>
                    <span className={`text-xs tracking-wider ${
                      isPathActive('/products', '@599') ? 'font-extrabold text-brand-pink' : 'font-bold text-brand-pink'
                    }`}>
                      @599 Store
                    </span>
                  </Link>

                  {/* Premium Fashion Link */}
                  <Link 
                    to="/products?tag=Premium" 
                    onClick={() => setIsMenuOpen(false)} 
                    className={`flex items-center space-x-3 px-3 py-3 rounded-xl transition-all group ${
                      isPathActive('/products', 'Premium') 
                        ? 'bg-brand-brown-light/80 text-brand-brown' 
                        : 'hover:bg-brand-pink-light'
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                      isPathActive('/products', 'Premium') 
                        ...
                        : 'bg-brand-brown-light text-brand-brown group-hover:bg-brand-brown group-hover:text-white'
                    }`}>
                      <Package className="w-4.5 h-4.5" />
                    </div>
                    <span className={`text-xs tracking-wider ${
                      isPathActive('/products', 'Premium') ? 'font-extrabold text-brand-brown' : 'font-bold text-brand-brown'
                    }`}>
                      Premium Fashion
                    </span>
                  </Link>
                      ...
                      {categories.map(cat => (
                        <Link 
                          ...
                          className={`flex items-center justify-between px-3 py-2.5 rounded-lg transition-all group ${
                            isPathActive('/products', undefined, cat.id)
                              ? 'bg-brand-pink-light/50 text-brand-pink'
                              : 'hover:bg-brand-pink-light text-neutral-600'
                          }`}
                        >
                          <span className={`text-[11px] ${
                            isPathActive('/products', undefined, cat.id) ? 'font-extrabold text-brand-pink' : 'font-bold text-neutral-600 group-hover:text-brand-pink'
                          }`}>{cat.name}</span>
                          <ChevronRight className={`w-4 h-4 transition-all ${
                            isPathActive('/products', undefined, cat.id) ? 'text-brand-pink translate-x-1' : 'text-neutral-400 group-hover:text-brand-pink group-hover:translate-x-1'
                          }`} />
                        </Link>
                      ))}
                    </div>
                  ...
                  <div className="pt-6 mt-4 border-t border-brand-pink/10">
                    ...
                    <Link 
                      ...
                      className={`flex items-center space-x-3 px-3 py-3 rounded-xl transition-all group ${
                        isPathActive('/account') 
                          ? 'bg-brand-pink-light text-brand-pink font-extrabold' 
                          : 'hover:bg-brand-pink-light text-neutral-700'
                      }`}
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                      ...
                      <span className={`text-xs tracking-wider ${
                        isPathActive('/account') ? 'font-extrabold text-brand-pink' : 'font-bold text-neutral-700 group-hover:text-brand-pink'
                      }`}>
                        Profile & Orders
                      </span>
                    </Link>
                    ...
                    >
                      ...
                      <span className={`text-xs tracking-wider ${
                        isPathActive('/wishlist') ? 'font-extrabold text-brand-pink' : 'font-bold text-neutral-700 dark:text-neutral-300 group-hover:text-brand-pink'
                      }`}>
                        Wishlist
                      </span>
                    </Link>
                  ...
                  {/* Theme Mode Capsule Toggle */}
                  <div className="pt-6 mt-4 border-t border-brand-pink/10 dark:border-neutral-800">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-3 px-3">Theme Mode</p>
                    <div className="bg-neutral-100 dark:bg-neutral-800 p-1 rounded-full flex relative">
                      <button
                        onClick={() => setTheme('light')}
                        className={`flex-1 flex items-center justify-center space-x-1 py-1.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider transition-all ${
                          theme === 'light'
...
export const BottomNav = () => {
  ...
  return (
    <motion.div 
      ...
      className="lg:hidden fixed bottom-4 inset-x-4 bg-white/70 dark:bg-neutral-950/65 backdrop-blur-2xl backdrop-saturate-150 border border-white/20 dark:border-neutral-800/40 z-50 px-6 py-3.5 rounded-2xl shadow-[0_12px_40px_0_rgba(0,0,0,0.12)] dark:shadow-[0_12px_40px_0_rgba(0,0,0,0.5)]"
    >
          ...
          <div>
            ...
            <ul className="space-y-3.5 text-xs text-neutral-400">
              <li className="flex items-center space-x-3">
                ...
                <span>+91 78880 00725</span>
              </li>
              <li className="flex items-center space-x-3">
                <Mail className="w-4 h-4 text-brand-pink" />
                <span>beingwomen2026@gmail.com</span>
              </li>
              <li className="flex items-start space-x-3">
                <MapPin className="w-4 h-4 mt-1 text-brand-pink flex-shrink-0" />
                <span>INNOV8 SOLITAIRE CORPORATE PARK, 7th Floor, S-11, Unit No. 1171, 1172, SOLITAIRE CORPORATE PARK, ANDHERI (E) Mumbai, Maharashtra-400093</span>
              </li>
            </ul>
          </div>
          <div>
