...
const safeJsonParse = (str: any, fallback: any[] = []): any[] => {
...
const safeJsonParseObj = (str: any, fallback: any = {}): any => {
  if (!str) return fallback;
  try {
...
function deductProductStock(productId: number, quantity: number, size: string | null, color: string | null) {
  const product: any = db.prepare('SELECT * FROM products WHERE id = ?').get(productId);
  if (!product) return;

  let variantStocks: any = {};
  try {
    variantStocks = product.variant_stock ? JSON.parse(product.variant_stock) : {};
  } catch (e) {
    variantStocks = {};
  }
  ...
  if (key && typeof variantStocks === 'object' && variantStocks !== null) {
    let matchedKey = '';
    if (variantStocks.hasOwnProperty(key)) {
      matchedKey = key;
    } else {
      const lowerKey = key.toLowerCase();
      const keys = Object.keys(variantStocks);
      matchedKey = keys.find(k => k.toLowerCase() === lowerKey) || '';
    }

    if (matchedKey) {
      const currentVal = Number(variantStocks[matchedKey]) || 0;
      variantStocks[matchedKey] = Math.max(0, currentVal - quantity);
    }
  }

  let newTotalStock = Math.max(0, product.stock_quantity - quantity);
  if (typeof variantStocks === 'object' && variantStocks !== null && Object.keys(variantStocks).length > 0) {
    newTotalStock = Object.values(variantStocks).reduce((sum: number, val: any) => sum + (Number(val) || 0), 0) as number;
  }

  db.prepare(`
    UPDATE products 
    SET stock_quantity = ?, variant_stock = ? 
    WHERE id = ?
  `).run(newTotalStock, JSON.stringify(variantStocks), productId);

  // Check and insert low stock or out of stock notifications
  if (newTotalStock === 0) {
    try {
      db.prepare(`
        INSERT INTO admin_notifications (type, title, message)
        VALUES (?, ?, ?)
      `).run('STOCK_OUT', 'Product Out of Stock', `"${product.name}" is now completely out of stock.`);
    } catch (e) {}
    ...
    try {
      db.prepare(`
        INSERT INTO admin_notifications (type, title, message)
        VALUES (?, ?, ?)
      `).run('STOCK_ALERT', 'Low Stock Alert', `Product "${product.name}" is running low. Only ${newTotalStock} left in stock.`);
    } catch (e) {}
  }
}
...
async function startServer() {
  ...
  app.post('/api/payment/create-order', authenticateToken, async (req: any, res) => {
    const { amount, items, shipping_address, phone, payment_method, full_name, email, coupon_code } = req.body;
    ...
    try {
      ...
      if (coupon_code) {
        const coupon: any = db.prepare('SELECT * FROM coupons WHERE code = ?').get(coupon_code.trim().toUpperCase());
        if (!coupon) {
          return res.status(400).json({ error: 'Invalid coupon code' });
        }
        const check = checkCouponEligibility(coupon, userId, databaseSubtotal);
        if (!check.valid) {
          return res.status(400).json({ error: check.error || 'Coupon is not valid for your order' });
        }
        couponId = coupon.id;
        discountApplied = check.discount || 0;
        freeDelivery = !!check.freeDelivery;
      }

      let shippingCharge = databaseSubtotal > 2000 ? 0 : 99;
      if (freeDelivery) {
        shippingCharge = 0;
      }

      const discountedSubtotal = Math.max(0, databaseSubtotal - discountApplied);
      const tax = Math.round(discountedSubtotal * 0.05);
      const databaseTotal = discountedSubtotal + shippingCharge + tax;
      ...
      if (Math.round(amount) !== Math.round(expectedAmount)) {
        return res.status(400).json({ error: `Payment amount mismatch: expected ₹${Math.round(expectedAmount)} but got ₹${Math.round(amount)}.` });
      }
    ...
    try {
      ...
      const executeOrderCreation = db.transaction(() => {
        ...
        for (const item of payload.items) {
          ...
          deductProductStock(item.product_id, item.quantity, item.size, item.color);
        }
        ...
        if (payload.coupon_id) {
          db.prepare(`
            ...
            VALUES (?, ?, ?, ?)
          `).run(payload.coupon_id, payload.user_id, orderId, payload.discount_applied || 0);
        }
  ...
  app.delete('/api/categories/:id', authenticateToken, isAdmin, (req, res) => {
    ...
    try {
      const deleteTransaction = db.transaction(() => {
        ...
        db.prepare('UPDATE sliders SET category_id = NULL WHERE category_id = ?').run(categoryId);
  ...
  app.get('/api/products', getOptionalUser, (req: any, res) => {
    ...
    const formattedProducts = products.map((p: any) => ({
      ...p,
      additional_images: safeJsonParse(p.additional_images),
      sizes: safeJsonParse(p.sizes),
      colors: safeJsonParse(p.colors),
      variant_stock: safeJsonParseObj(p.variant_stock, {}),
  ...
  app.get('/api/products/:id', getOptionalUser, (req: any, res) => {
    ...
    const formattedProduct = {
      ...product,
      additional_images: safeJsonParse(product.additional_images),
      sizes: safeJsonParse(product.sizes),
      colors: safeJsonParse(product.colors),
      variant_stock: safeJsonParseObj(product.variant_stock, {}),
  ...
  app.post('/api/products', authenticateToken, isAdmin, (req, res) => {
    const { name, category_id, main_image, additional_images, original_price, discount_price, stock_quantity, description, tag, is_cod_available, sizes, colors, new_arrival_days, variant_stock } = req.body;
    const normalizedCategoryId = category_id && category_id !== 'null' && category_id !== 'undefined' && String(category_id).trim() !== '' ? Number(category_id) : null;
    
    try {
      const result = db.prepare(`
        INSERT INTO products (name, category_id, main_image, additional_images, original_price, discount_price, stock_quantity, description, tag, is_cod_available, sizes, colors, new_arrival_days, variant_stock)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        name, normalizedCategoryId, main_image, JSON.stringify(additional_images || []), 
        ...
        new_arrival_days || 7, JSON.stringify(variant_stock || {})
      );
    ...
    try {
      db.prepare(`
        UPDATE products SET 
          ...
          new_arrival_days = ?, variant_stock = ?
        WHERE id = ?
      `).run(
        name, normalizedCategoryId, main_image, JSON.stringify(additional_images || []), 
        original_price, discount_price, stock_quantity, description, tag, 
  ...
  app.delete('/api/products/:id', authenticateToken, isAdmin, (req, res) => {
    ...
    try {
      const deleteTransaction = db.transaction(() => {
        ...
        db.prepare('UPDATE order_items SET product_id = NULL WHERE product_id = ?').run(productId);
  ...
  app.post('/api/sliders', authenticateToken, isAdmin, (req, res) => {
    const { desktop_banner, mobile_banner, category_id, description, show_description, button_text, show_button, desc_color, desc_position, button_position } = req.body;
    const normalizedCategoryId = category_id && category_id !== 'null' && category_id !== 'undefined' && String(category_id).trim() !== '' ? Number(category_id) : null;
    const normShowDesc = show_description ? 1 : 0;
    const normShowBtn = show_button ? 1 : 0;
    const normDescColor = desc_color || '#ffffff';
    const normDescPos = desc_position || 'middle-center';
    const normBtnPos = button_position || 'middle-center';
    
    try {
      const result = db.prepare('INSERT INTO sliders (desktop_banner, mobile_banner, category_id, description, show_description, button_text, show_button, desc_color, desc_position, button_position) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
        desktop_banner, mobile_banner, normalizedCategoryId, description || '', normShowDesc, button_text || '', normShowBtn, normDescColor, normDescPos, normBtnPos
      );
      const slider = db.prepare('SELECT * FROM sliders WHERE id = ?').get(result.lastInsertRowid);
      res.json(slider);
    } catch (error: any) {
      console.error('Error creating slider:', error);
      res.status(500).json({ error: error.message || 'Failed to create slider' });
    }
  });

  app.put('/api/sliders/:id', authenticateToken, isAdmin, (req, res) => {
    const { desktop_banner, mobile_banner, category_id, description, show_description, button_text, show_button, desc_color, desc_position, button_position } = req.body;
    const normalizedCategoryId = category_id && category_id !== 'null' && category_id !== 'undefined' && String(category_id).trim() !== '' ? Number(category_id) : null;
    const normShowDesc = show_description ? 1 : 0;
    const normShowBtn = show_button ? 1 : 0;
    ...
    try {
      db.prepare('UPDATE sliders SET desktop_banner = ?, mobile_banner = ?, category_id = ?, description = ?, show_description = ?, button_text = ?, show_button = ?, desc_color = ?, desc_position = ?, button_position = ? WHERE id = ?').run(
        desktop_banner, mobile_banner, normalizedCategoryId, description || '', normShowDesc, button_text || '', normShowBtn, normDescColor, normDescPos, normBtnPos, req.params.id
      );
      const slider = db.prepare('SELECT * FROM sliders WHERE id = ?').get(req.params.id);
      res.json(slider);
    } catch (error: any) {
      console.error('Error updating slider:', error);
      res.status(500).json({ error: error.message || 'Failed to update slider' });
    }
  });
  ...
  // --- Coupon Management Helpers & Routes ---

  function checkCouponEligibility(coupon: any, userId: number, cartSubtotal: number) {
    if (coupon.status !== 1) {
    ...
    if (coupon.start_date && now < new Date(coupon.start_date)) {
      return { valid: false, error: 'Coupon period has not started yet' };
    }
    ...
    if (cartSubtotal < coupon.min_order_amount) {
      return { valid: false, error: `Minimum order amount of ₹${coupon.min_order_amount} is required` };
    }

    const userUsage: any = db.prepare('SELECT COUNT(*) as count FROM coupon_usage WHERE coupon_id = ? AND user_id = ?').get(coupon.id, userId);
    const userUsageCount = userUsage ? userUsage.count : 0;
    if (userUsageCount >= coupon.user_usage_limit) {
      return { valid: false, error: 'You have exceeded the usage limit for this coupon' };
    }

    if (coupon.total_usage_limit !== null && coupon.total_usage_limit !== undefined && coupon.total_usage_limit > 0) {
      const totalUsage: any = db.prepare('SELECT COUNT(*) as count FROM coupon_usage WHERE coupon_id = ?').get(coupon.id);
      const totalUsageCount = totalUsage ? totalUsage.count : 0;
      if (totalUsageCount >= coupon.total_usage_limit) {
        return { valid: false, error: 'Total coupon usage limit has been reached' };
      }
    }

    let rules: any = {};
    try {
      rules = typeof coupon.eligibility === 'string' ? JSON.parse(coupon.eligibility) : coupon.eligibility;
    } catch (e) {
      rules = {};
    }
    ...
    const hasSpecificRules = rules.all_users === false && (
      rules.new_users ||
      rules.existing_users ||
      rules.never_used_coupon ||
      rules.premium_members ||
      (rules.specific_user_ids && rules.specific_user_ids.length > 0) ||
      (rules.specific_user_roles && rules.specific_user_roles.length > 0) ||
      (rules.specific_cities && rules.specific_cities.length > 0) ||
      (rules.specific_states && rules.specific_states.length > 0) ||
      (rules.specific_countries && rules.specific_countries.length > 0)
    );

    if (hasSpecificRules && !rules.all_users) {
      if (rules.new_users) {
        const ordersCountResult: any = db.prepare('SELECT COUNT(*) as count FROM orders WHERE user_id = ?').get(userId);
        const ordersCount = ordersCountResult ? ordersCountResult.count : 0;
        if (ordersCount > 0) {
          return { valid: false, error: 'Coupon is only for new users on their first order' };
        }
      }

      if (rules.existing_users) {
        const ordersCountResult: any = db.prepare('SELECT COUNT(*) as count FROM orders WHERE user_id = ?').get(userId);
        const ordersCount = ordersCountResult ? ordersCountResult.count : 0;
        if (ordersCount === 0) {
          return { valid: false, error: 'Coupon is only for existing users with past orders' };
        }
      }

      if (rules.never_used_coupon) {
        if (userUsageCount > 0) {
          return { valid: false, error: 'You have already used this coupon' };
        }
      }

      if (rules.premium_members) {
        if (user.role !== 'premium') {
          return { valid: false, error: 'Coupon is for premium members only' };
        }
      }

      if (rules.specific_user_ids && rules.specific_user_ids.length > 0) {
        const matched = rules.specific_user_ids.some((val: any) => {
          const s = String(val).trim().toLowerCase();
          return s === String(user.id) || s === String(user.email).trim().toLowerCase();
        });
        if (!matched) {
          return { valid: false, error: 'Your account is not eligible for this coupon' };
        }
      }

      if (rules.specific_user_roles && rules.specific_user_roles.length > 0) {
        const matched = rules.specific_user_roles.some((r: string) => r.trim().toLowerCase() === String(user.role).trim().toLowerCase());
        if (!matched) {
          return { valid: false, error: 'Your user role is not eligible for this coupon' };
        }
      }

      if (rules.specific_cities && rules.specific_cities.length > 0) {
        const fullAddr = String(user.address || '').toLowerCase();
        const matched = rules.specific_cities.some((city: string) => fullAddr.includes(city.trim().toLowerCase()));
        if (!matched) {
          return { valid: false, error: 'Coupon is not eligible for your city' };
        }
      }

      if (rules.specific_states && rules.specific_states.length > 0) {
        const userState = String(user.state || '').trim().toLowerCase();
        const fullAddr = String(user.address || '').toLowerCase();
        const matched = rules.specific_states.some((state: string) => {
          const s = state.trim().toLowerCase();
          return userState === s || fullAddr.includes(s);
        });
        if (!matched) {
          return { valid: false, error: 'Coupon is not eligible for your state' };
        }
      }

      if (rules.specific_countries && rules.specific_countries.length > 0) {
        const fullAddr = String(user.address || '').toLowerCase();
        const matched = rules.specific_countries.some((c: string) => fullAddr.includes(c.trim().toLowerCase()));
        if (!matched) {
          return { valid: false, error: 'Coupon is not eligible for your country' };
        }
      }
    }
    ...
    if (coupon.discount_type === 'flat') {
      ...
      discount = cartSubtotal * (Number(coupon.discount_value) / 100);
    ...
    return { valid: true, discount: Math.round(discount), freeDelivery };
  }

  function getEligibleUsersCount(coupon: any) {
    const users = db.prepare('SELECT id, email, role, address, state FROM users').all();
    let count = 0;
    for (const u of users) {
      const res = checkCouponEligibility(coupon, u.id, coupon.min_order_amount || 0);
      if (res.valid) {
        count++;
      }
    }
  ...
  // Admin GET coupons (with search and filter)
  app.get('/api/coupons', authenticateToken, isAdmin, (req, res) => {
    ...
    const hydratedCoupons = coupons.map((c: any) => {
      const usage: any = db.prepare('SELECT COUNT(*) as count FROM coupon_usage WHERE coupon_id = ?').get(c.id);
      ...
      return {
        ...
        eligible_user_count: getEligibleUsersCount(c)
      };
    });

    res.json(hydratedCoupons);
  });

  // Public/Authenticated GET available coupons for the current user
  app.get('/api/coupons/available', authenticateToken, (req: any, res) => {
    const userId = req.user.id;
    const cartSubtotal = Number(req.query.cart_subtotal) || 0;
    const coupons = db.prepare('SELECT * FROM coupons WHERE status = 1').all();
    const available: any[] = [];

    for (const c of coupons) {
      const check = checkCouponEligibility(c, userId, cartSubtotal);
      // We return coupons that are fully eligible, OR coupons that are active but just need more subtotal amount (near-eligible)
      // So we can show the user both, but mark clearly which ones are instantly applicable.
      available.push({
        ...c,
        eligibility: typeof c.eligibility === 'string' ? JSON.parse(c.eligibility) : c.eligibility,
        checkResult: check
  ...
  // Admin GET single coupon with details and usage history
  app.get('/api/coupons/:id', authenticateToken, isAdmin, (req, res) => {
    const coupon: any = db.prepare('SELECT * FROM coupons WHERE id = ?').get(req.params.id);
    if (!coupon) {
      return res.status(404).json({ error: 'Coupon not found' });
    }

    const usages = db.prepare(`
      SELECT cu.*, u.email as user_email, u.full_name as user_name, o.created_at as order_date
    ...
    const usageCountResult: any = db.prepare('SELECT COUNT(*) as count FROM coupon_usage WHERE coupon_id = ?').get(coupon.id);

    res.json({
      ...coupon,
      eligibility: typeof coupon.eligibility === 'string' ? JSON.parse(coupon.eligibility) : coupon.eligibility,
      usages,
      total_usage_count: usageCountResult ? usageCountResult.count : 0,
      eligible_user_count: getEligibleUsersCount(coupon)
    });
  });

  // Admin POST create coupon
  app.post('/api/coupons', authenticateToken, isAdmin, (req, res) => {
    const {
      title,
      code,
      description,
      ...
      user_usage_limit,
      total_usage_limit,
      one_coupon_per_order,
      start_date,
      end_date
    } = req.body;

    if (!title || !code || !discount_type) {
      return res.status(400).json({ error: 'Title, Coupon Code and Discount Type are required' });
    }
    ...
    try {
      ...
      const result = db.prepare(`
        INSERT INTO coupons (
          title, code, description, status, discount_type, discount_value,
          max_discount, min_order_amount, eligibility, user_usage_limit,
          total_usage_limit, one_coupon_per_order, start_date, end_date
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        title,
        ...
        eligibilityStr || '{}',
        user_usage_limit === undefined ? 1 : Number(user_usage_limit),
        total_usage_limit ? Number(total_usage_limit) : null,
        one_coupon_per_order === undefined ? 1 : (one_coupon_per_order ? 1 : 0),
        start_date || null,
        end_date || null
      );
  ...
  // Admin PUT edit coupon
  app.put('/api/coupons/:id', authenticateToken, isAdmin, (req, res) => {
    const {
      title,
      code,
      description,
    ...
    try {
      ...
      db.prepare(`
        UPDATE coupons SET
          title = ?, code = ?, description = ?, status = ?, discount_type = ?,
          discount_value = ?, max_discount = ?, min_order_amount = ?, eligibility = ?,
          user_usage_limit = ?, total_usage_limit = ?, one_coupon_per_order = ?,
          start_date = ?, end_date = ?
        WHERE id = ?
      `).run(
        title,
        ...
        Number(min_order_amount) || 0,
        eligibilityStr || '{}',
        Number(user_usage_limit) || 1,
        total_usage_limit ? Number(total_usage_limit) : null,
        one_coupon_per_order ? 1 : 0,
        start_date || null,
        end_date || null,
        req.params.id
      );
  ...
  // Admin PUT toggle status
  app.put('/api/coupons/:id/toggle', authenticateToken, isAdmin, (req, res) => {
    const { status } = req.body;
    try {
      db.prepare('UPDATE coupons SET status = ? WHERE id = ?').run(status ? 1 : 0, req.params.id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  // Admin DELETE coupon
  app.delete('/api/coupons/:id', authenticateToken, isAdmin, (req, res) => {
    try {
      db.prepare('DELETE FROM coupons WHERE id = ?').run(req.params.id);
      db.prepare('DELETE FROM coupon_usage WHERE coupon_id = ?').run(req.params.id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  // Public/Authenticated POST apply coupon
  app.post('/api/coupons/apply', authenticateToken, (req: any, res) => {
    ...
    const coupon: any = db.prepare('SELECT * FROM coupons WHERE code = ?').get(code.trim().toUpperCase());
    if (!coupon) {
      return res.status(404).json({ error: 'Invalid coupon code' });
    }

    const check = checkCouponEligibility(coupon, userId, Number(cart_subtotal) || 0);
    if (!check.valid) {
